...
bool ManualLoopCloseModal::set_end_keyframe(int keyframe_id) {
  ...
  auto found = graph->keyframes.find(keyframe_id);
  if (found == graph->keyframes.end() || begin_keyframe == nullptr) {
    return false;
  }
...
bool ManualLoopCloseModal::is_oversize_voxel_capacity(pcl::PointCloud<pcl::PointXYZI> pointCloud, float resolution) {

    pcl::PointXYZI minPoint, maxPoint;
    pcl::getMinMax3D(pointCloud, minPoint, maxPoint);
    std::cout << "Register resolution: " << resolution << std::endl;

    // compute voxel indices capacity
    Eigen::Vector3f delta_p = (maxPoint.getVector3fMap() - minPoint.getVector3fMap())/resolution;
    int voxel_capacity = 1;
    for(int i=0; i < delta_p.size(); ++i) {
        voxel_capacity *= delta_p[i];
    }
    // check whether overflow in the number of voxels
    if (voxel_capacity < 0 || voxel_capacity > INT_MAX)
        return true;
    return false;
}
  ...
  if (ImGui::BeginPopupModal("scan matching", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::Button("OK")) {
        try {
            ...
            bool target_voxel_capacity = is_oversize_voxel_capacity(*begin_keyframe->lock()->cloud, registration_method.getRegistrationResolution());
            bool source_voxel_capacity = is_oversize_voxel_capacity(*end_keyframe->lock()->cloud, registration_method.getRegistrationResolution());

            if (target_voxel_capacity || source_voxel_capacity) {
                registration_method.setErrorMessage("Resolution is too small.");
            } else {
                registration_method.setErrorMessage("");
            ...
            registration_method.setErrorMessage(e.what());
        }
