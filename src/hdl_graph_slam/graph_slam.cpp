...
g2o::VertexSE3* GraphSLAM::add_se3_node(const Eigen::Isometry3d& pose) {
  ...
  vertex->setId(static_cast<int>(++max_vertex_id));
...
int GraphSLAM::optimize(int num_iterations) {
  ...
  std::cout << "time: " << boost::format("%.3f") % (t2 - t1).count() << "[sec]" << std::endl;
...
bool GraphSLAM::load(const std::string& filename, Box3D &box) {
    ...
    for (auto it = graph->vertices().begin(); it != graph->vertices().end(); ++it)
    {
        ...
        {
            count = 1;
            box.max.x = vertex->estimate().translation().x();
            box.min.x = box.max.x;

            box.max.y = vertex->estimate().translation().y();
            box.min.y = box.max.y;

            box.max.z = vertex->estimate().translation().z();
            box.min.z = box.max.z;
        }

        box.max.x = std::max(box.max.x, vertex->estimate().translation().x());
        box.max.y = std::max(box.max.y, vertex->estimate().translation().y());
        box.max.z = std::max(box.max.z, vertex->estimate().translation().z());

        box.min.x = std::min(box.min.x, vertex->estimate().translation().x());
        box.min.y = std::min(box.min.y, vertex->estimate().translation().y());
        box.min.z = std::min(box.min.z, vertex->estimate().translation().z());
    }
