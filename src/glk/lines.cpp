...
Lines::Lines(float line_width,
		...
		const std::vector<Eigen::Vector4f,
				Eigen::aligned_allocator<Eigen::Vector4f>> &colors,
		const std::vector<Eigen::Vector4i,
				Eigen::aligned_allocator<Eigen::Vector4i>> &infos) :
	...
	for (int i = 0; i < vertices.size(); i += 2) {
		...
		Eigen::Vector3f axis =
				std::abs(direction.normalized().dot(Eigen::Vector3f::UnitZ()))
						< 0.9f ?
	...
	if (!infos.empty()) {
		std::vector<Eigen::Vector4i, Eigen::aligned_allocator<Eigen::Vector4i>> infos_ext(
				infos.size() * 4);
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(int) * infos_ext.size() * 4,
				infos_ext.data(), GL_STATIC_DRAW);
	}
	...
	std::vector<int> sub_indices = { 0, 1, 4, 1, 5, 4, 4, 5, 2, 5, 3, 2, 2, 3,
			6, 3, 6, 7, 6, 7, 0, 7, 1, 0 };
