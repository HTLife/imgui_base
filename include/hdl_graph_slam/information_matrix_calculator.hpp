...
public:
  ...
  InformationMatrixCalculator(NodeHandle& nh);
  ~InformationMatrixCalculator();

  template<typename ParamServer>
  void load(ParamServer& params) {
    node.use_const_inf_matrix = params.template param<bool>("use_const_inf_matrix", false);
    node.const_stddev_x = params.template param<double>("const_stddev_x", 0.5);
    node.const_stddev_q = params.template param<double>("const_stddev_q", 0.1);

    node.var_gain_a = params.template param<double>("var_gain_a", 20.0);
    node.min_stddev_x = params.template param<double>("min_stddev_x", 0.1);
    node.max_stddev_x = params.template param<double>("max_stddev_x", 5.0);
    node.min_stddev_q = params.template param<double>("min_stddev_q", 0.05);
    node.max_stddev_q = params.template param<double>("max_stddev_q", 0.2);
    node.fitness_score_thresh = params.template param<double>("fitness_score_thresh", 2.5);
  }
