...
public:
  ...
  virtual void draw(const DrawFlags& flags, glk::GLSLShader& shader, int id, const Eigen::Vector4f& color_) override {
      if (!flags.draw_edges || !flags.draw_se3_edges) {
        return;
      }
      ...
      Eigen::Vector4f color(0.95f, 0.0f, 0.0f, 1.0f);
      Eigen::Vector4f color2(color_);
      Eigen::Vector4i info(EDGE, edge->id(), 0, 0);
      if (v1->id() == id)
    	  line_buffer.add_line(p1, p2, color2, color, info);
      else if (v2->id() == id)
    	  line_buffer.add_line(p1, p2, color, color2, info);
      else
    	  line_buffer.add_line(p1, p2, color, color, info);
    }
private:
