...
public:
  ...
  bool load_map_data(const std::string& directory, guik::ProgressInterface& progress, Box3D &box);
  ...
  std::vector<InteractiveGraph*> split_map_data();
  ...
  Eigen::Vector3f get_vertex(int id);
  void drag_vertex(int id, int axis, double delta, float speed);
  void drag_vertex(int id, double delta1, double delta2, float speed);
  void rotate_vertex(int id, int axis, double delta, float speed);
  void move_vertex(int id, Eigen::Vector3f new_pos);

  void drag_map(int flag1, int flag2, int axis, double delta, float speed);
  void drag_map(int flag1, int flag2, double delta1, double delta2, float speed);
  void rotate_map(int flag1, int flag2,int axis, double delta, float speed);
  ...
  bool save_pointcloud(const std::string& filename, guik::ProgressInterface& progress, float *voxel_leaf_sizes, int skip_frame=1);
...
public:
  ...
  std::vector<long>vector_max_id_maps;
